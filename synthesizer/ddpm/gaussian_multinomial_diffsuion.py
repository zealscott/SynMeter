...
class GaussianMultinomialDiffusion(torch.nn.Module):
    ...
    #### the below functions are for auditing the sample process of TabDDPM ####
    @torch.no_grad()
    def sample_auditing(self, num_samples, y_dist, store_dir, interval=10):
        """
        this function is to auditing the sample process of TabDDPM
        each interval, the model output the current sample data
        """
        b = num_samples
        device = self.log_alpha.device
        z_norm = torch.randn((b, self.num_numerical_features), device=device)
        ...
        out_dict = {'y': y.long().to(device)}
        saved_interval = self.num_timesteps // interval
        for i in reversed(range(0, self.num_timesteps)):
            print(f'Sample timestep {i:4d}', end='\r')
            t = torch.full((b,), i, device=device, dtype=torch.long)
            model_out = self._denoise_fn(
            ...
            if i % saved_interval == 0:
                ...
                gen_x = sample
                gen_y = out_dict['y'].cpu()
                sampled = self.data_transformer.inverse_transform(gen_x, gen_y)
